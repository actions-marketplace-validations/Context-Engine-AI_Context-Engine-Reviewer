...
describe('comments utilities', () => {
  ...
  test('isThreadRelevant checks signature and mentions', () => {
    ...
    const withReviewerMention: ReviewCommentThread = {
      ...base,
      comments: [{ id: 1, body: 'please @context-engine-reviewer check', user: { login: 'u' }, path: 'f.ts' }] as any,
    };
    expect(isThreadRelevant(withReviewerMention)).toBe(true);

    const withShortReviewerMention: ReviewCommentThread = {
      ...base,
      comments: [{ id: 1, body: 'ping @ce-reviewer', user: { login: 'u' }, path: 'f.ts' }] as any,
    };
    expect(isThreadRelevant(withShortReviewerMention)).toBe(true);
  });
});
